...
static std::vector<Token> tokenize(const std::string& s) {
    ...
    while (i < s.size()) {
        ...
        if (std::isupper((unsigned char) s[i])) {
            size_t start = i;
            while (i < s.size() && std::isupper((unsigned char) s[i]))
                i++;
            ...
            if (w == "AND")
                ...
                out.push_back({VAR, w[0]});
            else {
                std::cerr << "Unknown token: " << w << "\n";
                std::exit(1);
            }
            continue;
        }
...
static int parse_not() {
    if (pos < tokens.size() && tokens[pos].type == NOT) {
        pos++;
        return !parse_not();
    }
    return parse_atom();
...
int main() {
    ...
    if (tokens[pos].type != TEOF) {
        std::cerr << "Unexpected trailing tokens\n";
        return 1;
    }
    return 0;
}
